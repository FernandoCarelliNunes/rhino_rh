...
@login_required
def sincronizar_sophia(request, vaga_id):
    vaga = get_object_or_404(Vaga, id=vaga_id)
    
    # --- CONFIGURAÇÃO DA API STARMIND ---
    # Substitua pelo seu token real fornecido pela plataforma
    API_TOKEN = "SEU_TOKEN_AQUI" 
    URL_BASE = "https://docs.starmind.com/api/v1/assessments" # Ajustado conforme endpoint da doc
    ...
    try:
        # 1. Faz a chamada para a API buscando os candidatos da vaga específica
        # Geralmente passamos um ID externo ou nome da vaga como filtro
        response = requests.get(f"{URL_BASE}?vaga_id={vaga.id}", headers=headers, timeout=10)
        
        if response.status_code == 200:
            dados = response.json()
            candidatos_api = dados.get('data', []) # Onde os perfis costumam vir
            
            contagem_novos = 0
            
            for item in candidatos_api:
                # 2. get_or_create: Se o email ou ID da IA já existir, ele pula. 
                # Se não existir, ele cadastra sozinho!
                obj, created = Candidato.objects.get_or_create(
                    email=item.get('email'),
                    vaga=vaga,
                    defaults={
                        'nome': item.get('name'),
                        'status_candidato': 'Triagem', # Status inicial no seu sistema
                        # Se você criou campos extras para IA:
                        # 'score_ia': item.get('assessment_score'),
                    }
                )
                if created:
                    contagem_novos += 1
            ...
            if contagem_novos > 0:
                messages.success(request, f"🚀 Sucesso! {contagem_novos} novos candidatos importados da Sophia.")
            else:
                messages.info(request, "A lista já está atualizada. Nenhum candidato novo encontrado.")
                
        else:
            messages.error(request, f"Erro na API Starmind: {response.status_code}")
